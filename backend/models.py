    ...
    def create_profile(db, doctor_id, profile_data, document_paths):
        ...
        try:
            ...
            profile = {
                ...
                'experience_years': int(profile_data.get('experienceYears', 0)),
                'consultationFee': float(profile_data.get('consultationFee', 0)),
                'chatConsultationFee': float(profile_data.get('chatConsultationFee', 0)),
                'bio': profile_data.get('bio', ''),
                'languages': profile_data.get('languages', []),
                'qualifications': profile_data.get('qualifications', []),
                'clinic_address': profile_data.get('clinicAddress', ''),
...
class Appointment(BaseModel):
    ...
    def create(db, data):
        ...
        appointment = {
            ...
            'google_meet_link': data.get('google_meet_link', ''),  # Google Meet link field
            'created_at': datetime.utcnow(),
            'updated_at': datetime.utcnow()
        }
    ...
    @staticmethod
    def update_google_meet_link(db, appointment_id, google_meet_link):
        """Update Google Meet link for appointment"""
        try:
            result = db[Appointment.collection_name].update_one(
                {'_id': ObjectId(appointment_id)},
                {
            ...
            print(f"Error updating Google Meet link: {str(e)}")
            return False
    ...
    def find_by_id(db, appointment_id):
        ...
        try:
            ...
            if not ObjectId.is_valid(appointment_id):
                print(f"Invalid appointment ID format: {appointment_id}")
                return None
...
class ChatMessage(BaseModel):
    """Chat message model for secure end-to-end encrypted messages"""
    collection_name = 'chat_messages'
    
    @staticmethod
    def create(db, data):
        """Create a new encrypted chat message
        
        The message content should already be encrypted on the client side
        before being sent to the server. The server only stores the encrypted
        message and doesn't have the ability to decrypt it.
        """
        import hashlib
        ...
        message = {
            ...
            'encrypted_content': data.get('encrypted_content'),  # Already encrypted by client
            'content_type': data.get('content_type', 'text'),  # text, image, file
            'file_metadata': data.get('file_metadata', {}),  # For attachments: name, size, type
            'read_by_recipient': False,
            'created_at': datetime.utcnow(),
            'message_hash': hashlib.sha256(data.get('encrypted_content', '').encode()).hexdigest()
        }
        
        # Insert message
        result = db[ChatMessage.collection_name].insert_one(message)
        message['_id'] = result.inserted_id
        
        return ChatMessage.serialize_id(message)
    
    @staticmethod
    def find_by_appointment(db, appointment_id, limit=100, skip=0):
        """Find all messages for a specific appointment, with pagination"""
        try:
            messages = list(db[ChatMessage.collection_name]
                .find({'appointment_id': appointment_id})
                .sort('created_at', 1)
                .skip(skip)
                .limit(limit))
            
            return ChatMessage.serialize_list(messages)
        except Exception as e:
            print(f"Error finding chat messages: {str(e)}")
            return []
    ...
    def mark_as_read(db, message_id, user_id):
        ...
        try:
            result = db[ChatMessage.collection_name].update_one(
                {'_id': ObjectId(message_id)},
                {'$set': {'read_by_recipient': True, 'read_at': datetime.utcnow()}}
            )
            return result.modified_count > 0
        except Exception as e:
    ...
    @staticmethod
    def get_unread_count(db, appointment_id, user_id):
        """Get count of unread messages for a user in an appointment"""
        try:
            # Find messages sent to this user (where they are not the sender) that are unread
            count = db[ChatMessage.collection_name].count_documents({
    ...
    @staticmethod
    def verify_message_integrity(db, message_id, encrypted_content):
        """Verify message integrity using hash comparison"""
        import hashlib
        
        try:
            message = db[ChatMessage.collection_name].find_one({'_id': ObjectId(message_id)})
            if not message:
                return False
                
            # Compute hash of provided content
            computed_hash = hashlib.sha256(encrypted_content.encode()).hexdigest()
            
            # Compare with stored hash
            return computed_hash == message.get('message_hash')
        except Exception as e:
            print(f"Error verifying message integrity: {str(e)}")
            return False 