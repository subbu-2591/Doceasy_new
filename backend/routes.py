from flask import Blueprint, request, jsonify, current_app, send_file, send_from_directory, session, redirect, url_for
from functools import wraps
import jwt
from datetime import datetime, timedelta
from models import Admin, Doctor, Patient, Complaint, Notification, Appointment, User, SystemSettings, DoctorAvailability, Payment, WebRTCRoom, ChatMessage
...
def create_doctor_profile(current_user):
    ...
    try:
        ...
        profile_data = {
            ...
            'chatConsultationFee': request.form.get('chatConsultationFee', request.form.get('consultationFee')),  # Default to regular fee
...
def update_doctor_profile(current_user):
    ...
    try:
            ...
            if request.form.get('experience_years'):
                try:
                    ...
                    if experience_years is not None:
                        update_data['experience_years'] = int(experience_years)
                except (ValueError, TypeError):
                    pass
            if request.form.get('consultationFee'):
                try:
                    consultation_fee = request.form.get('consultationFee')
                    if consultation_fee is not None:
                        update_data['consultationFee'] = float(consultation_fee)
                except (ValueError, TypeError):
                    pass
            if request.form.get('chatConsultationFee'):
                try:
                    chat_fee = request.form.get('chatConsultationFee')
                    if chat_fee is not None:
                        update_data['chatConsultationFee'] = float(chat_fee)
                except (ValueError, TypeError):
                    pass
            else:
                # Default chat consultation fee to regular fee if not specified
                if request.form.get('consultationFee'):
                    try:
                        consultation_fee = request.form.get('consultationFee')
                        if consultation_fee is not None:
                            update_data['chatConsultationFee'] = float(consultation_fee)
                    except (ValueError, TypeError):
                        pass
        ...
        # Handle JSON data (for application/json requests)
            ...
            data = request.get_json()
            if data:
                for field in ['first_name', 'last_name', 'phone', 'specialty', 'experience_years', 'consultationFee', 'chatConsultationFee', 'bio']:
                    if field in data and data[field] is not None:
                        if field in ['experience_years', 'consultationFee', 'chatConsultationFee']:
                            try:
                                update_data[field] = float(data[field]) if field in ['consultationFee', 'chatConsultationFee'] else int(data[field])
                            except (ValueError, TypeError):
                                continue
                        else:
                ...
                # Default chat consultation fee to regular fee if not specified
                if 'consultationFee' in update_data and 'chatConsultationFee' not in update_data:
                    update_data['chatConsultationFee'] = update_data['consultationFee']
        ...
        if result.modified_count > 0:
            ...
            if updated_doctor is not None:
                ...
                return jsonify({'message': 'Profile updated but unable to retrieve updated data'}), 200
        else:
...
@doctor_required
def get_doctor_patient_details(current_user, patient_id):
...
def update_patient_profile(current_user):
    ...
    try:
        ...
        if result.modified_count > 0:
            ...
            # Remove sensitive information
            # Fix for patient profile update issue
...
def join_consultation(current_user, appointment_id):
    ...
    try:
        ...
        appointment = Appointment.find_by_id(db, appointment_id)
        if not appointment:
            logger.warning(f"Appointment not found in join_consultation: {appointment_id}")
            return jsonify({'error': 'Appointment not found'}), 404
        
        # Check if user is part of this appointment
        if (current_user.get('role') == 'patient' and str(appointment.get('patient_id')) != str(current_user.get('id')) or
            current_user.get('role') == 'doctor' and str(appointment.get('doctor_id')) != str(current_user.get('id'))):
            return jsonify({'error': 'Access denied'}), 403
        ...
        payment = Payment.find_by_appointment_id(db, appointment_id)
        if not payment:
            # If no payment found, we'll still allow access but log a warning
            logger.warning(f"No payment found for appointment: {appointment_id}")
        ...
        consultation_data = {
            ...
            'phone_number': appointment.get('patient_phone') if current_user.get('role') == 'doctor' else None,
            'google_meet_link': appointment.get('google_meet_link', ''),
            'urgent': appointment.get('urgent', False)
        }
...
@admin_bp.route('/appointments/<appointment_id>/meet-link', methods=['PUT'])
@admin_required
def update_google_meet_link(current_user, appointment_id):
    ...
    try:
        ...
        success = Appointment.update_google_meet_link(db, appointment_id, google_meet_link)
        
        if not success:
            return jsonify({'error': 'Failed to update Google Meet link'}), 500
            
        # Get updated appointment data
        updated_appointment = Appointment.find_by_id(db, appointment_id)
        ...
        if doctor_id:
            Notification.create(db, {
                'title': 'Video Call Details Updated',
                'message': f"Google Meet link has been added to your appointment with {appointment.get('patient_name')} on {appointment.get('appointment_date')}",
                'type': 'info',
                'userId': doctor_id,
                'relatedTo': {
                    'type': 'appointment',
        ...
        if patient_id:
            Notification.create(db, {
                'title': 'Video Call Details Updated',
                'message': f"Google Meet link has been added to your appointment with Dr. {appointment.get('doctor_name')} on {appointment.get('appointment_date')}",
                'type': 'info',
                'userId': patient_id,
                'relatedTo': {
                    'type': 'appointment',
        ...
        logger.error(f"Error updating Google Meet link: {str(e)}")
        return jsonify({'error': 'An error occurred while updating Google Meet link'}), 500

# Chat routes
@api_bp.route('/chat/messages/<appointment_id>', methods=['GET'])
@token_required
def get_chat_messages(current_user, appointment_id):
    ...
    try:
        ...
        appointment = Appointment.find_by_id(db, appointment_id)
        if not appointment:
            # Return empty messages list instead of error to prevent frontend from breaking
            # This handles the case where the appointment doesn't exist yet
            logger.warning(f"Appointment not found for chat messages: {appointment_id}")
            return jsonify({
                'success': True,
                'messages': []
            }), 200
        ...
        # Check if user is part of this appointment
        if current_user.get('role') == 'doctor':
            if str(appointment.get('doctor_id')) != str(current_user.get('id')):
                return jsonify({'error': 'Unauthorized access'}), 403
        elif current_user.get('role') == 'patient':
            if str(appointment.get('patient_id')) != str(current_user.get('id')):
                return jsonify({'error': 'Unauthorized access'}), 403
        ...
        # Get messages
        messages = ChatMessage.find_by_appointment(db, appointment_id, limit, skip)
...
def send_chat_message(current_user):
    ...
    try:
        ...
        # Validate required fields
        required_fields = ['appointment_id', 'encrypted_content']
        missing_fields = []
        for field in required_fields:
        ...
        if appointment.get('consultation_type') != 'chat':
            return jsonify({'error': 'This is not a chat consultation'}), 400
        ...
        # Create message
        message = ChatMessage.create(db, message_data)
        
        # Create notification for recipient
        recipient_id = appointment.get('doctor_id') if sender_type == 'patient' else appointment.get('patient_id')
        recipient_type = 'doctor' if sender_type == 'patient' else 'patient'
        
        # Customize notification based on urgency
        is_urgent = appointment.get('urgent', False)
        
        notification_data = {
            'user_id': recipient_id,
            'user_type': recipient_type,
            'title': 'New chat message' + (' (URGENT)' if is_urgent else ''),
            'message': f"You have a new message in your {'URGENT ' if is_urgent else ''}chat consultation",
            'type': 'chat_message',
        ...
        Notification.create(db, notification_data)
...
@api_bp.route('/chat/messages/<message_id>/read', methods=['POST'])
@token_required
def mark_message_read(current_user, message_id):
    ...
    try:
        ...
        result = ChatMessage.mark_as_read(db, message_id, current_user.get('id'))
...
@api_bp.route('/chat/messages/<appointment_id>/unread', methods=['GET'])
@token_required
def get_unread_count(current_user, appointment_id):
    ...
    try:
        ...
        # Get unread count
        count = ChatMessage.get_unread_count(db, appointment_id, current_user.get('id'))
...
@token_required
def upload_chat_file(current_user):
