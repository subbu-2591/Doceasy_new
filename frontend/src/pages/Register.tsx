...
// API base URL - using centralized httpClient
...
const Register = () => {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Create registration payload (only including fields expected by the backend)
      ...
      // Try a ping request first to test connectivity
      try {
        const pingResponse = await httpClient.get(`${API_BASE_URL}/auth/ping`);
        console.log("Server ping successful:", pingResponse.data);
      } catch (pingError) {
        console.error("Server ping failed:", pingError);
      }
      
      const response = await httpClient.post(`${API_BASE_URL}/auth/register`, registrationData);
  ...
  const handleVerifyOTP = async (e: React.FormEvent) => {
    ...
    try {
      const response = await httpClient.post(`${API_BASE_URL}/auth/verify-otp`, {
        user_id: userId,
        otp: otp
      });
                    ...
                    <div className="space-y-4">
                      ...
                      <p className="text-sm text-gray-500 mt-2">
                        ...
                        <button 
                          ...
                          onClick={async () => {
                            try {
                              const response = await httpClient.post(`${API_BASE_URL}/auth/resend-otp`, { user_id: userId });
