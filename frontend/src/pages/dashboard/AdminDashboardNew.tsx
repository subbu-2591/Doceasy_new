...
import { 
  ...
  LogOut, UserCog, FileBarChart, RefreshCw, Clock, History, Video
} from "lucide-react";
...
const AdminDashboardNew = () => {
  ...
  const [isUpdatingMeetLink, setIsUpdatingMeetLink] = useState(false);
  ...
  // Initialize Google Meet link when a consultation is selected
  useEffect(() => {
    if (selectedConsultation?.google_meet_link) {
      setGoogleMeetLink(selectedConsultation.google_meet_link);
    } else {
      setGoogleMeetLink("");
    }
                ...
                <div className="space-y-4 py-2">
                  ...
                  {selectedConsultation.consultationType === "video" && (
                    <div>
                      ...
                      <div className="space-y-3 bg-purple-50 p-3 rounded-md text-sm">
                        <div className="grid grid-cols-1 gap-2">
                          <div className="font-medium">Google Meet Link:</div>
                          <div className="flex gap-2 items-center">
                            <Input
                              placeholder="Add Google Meet link here (e.g., https://meet.google.com/abc-defg-hij)"
                            ...
                            <Button 
                              onClick={async () => {
                                if (!googleMeetLink) {
                                  toast({
                                    title: "Please enter a Google Meet link",
                                    description: "You need to provide a valid Google Meet URL",
                                    variant: "destructive"
                                  });
                                  return;
                                }
                                ...
                                try {
                                  await adminService.updateGoogleMeetLink(selectedConsultation.id, googleMeetLink);
                                  toast({
                                    title: "Google Meet link updated",
                                    description: "The link has been shared with the doctor and patient",
                                  });
                                  ...
                                  toast({
                                    title: "Failed to update Google Meet link",
                          ...
                          {selectedConsultation.google_meet_link && (
                            <div className="mt-2 p-2 bg-white rounded-md border border-purple-200">
                              <div className="flex justify-between items-center">
                                <span className="font-medium text-purple-700">Current Google Meet Link:</span>
                                <a 
