...
const DoctorDashboardNew = () => {
  ...
  const fetchTodayAppointments = async () => {
    ...
    try {
      ...
      // Add detailed logging to see what's coming from the API
      ...
      console.log('Chat appointments only:', todayAppts.filter(a => a.consultation_type === 'chat'));
  ...
  // Start consultation based on type (video, voice, or chat)
  const startConsultation = (appointment: any) => {
    ...
    if (appointment.consultation_type === 'chat') {
      ...
      // Default to video call for all other types (video, voice, phone)
  ...
  useEffect(() => {
    if (profileData && !editingProfile) {
      setProfileFormData({
        ...
        chatConsultationFee: profileData.chatConsultationFee || profileData.consultationFee || '',
        bio: profileData.bio || '',
      });
  ...
  const canJoinConsultation = (appointmentDate: string, isUrgent = false): { canJoin: boolean; message: string } => {
    // For urgent consultations, always allow joining
    if (isUrgent) {
      return {
        canJoin: true,
        message: 'Join Urgent Consultation'
      };
    }
                ...
                <div className="space-y-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
                        <Label htmlFor="consultationFee">Video Consultation Fee (₹)</Label>
                        <Input
                          id="consultationFee"
                          type="number"
                          placeholder="1500"
                      ...
                      <div>
                        <Label htmlFor="chatConsultationFee">Chat Consultation Fee (₹)</Label>
                        <Input
                          id="chatConsultationFee"
                          type="number"
                          placeholder="1000"
                          value={profileFormData.chatConsultationFee || ''}
                          onChange={(e) => handleProfileFormChange('chatConsultationFee', e.target.value)}
                          disabled={!editingProfile}
                          className={!editingProfile ? 'bg-gray-50' : ''}
                        />
                      </div>
                          ...
                          <div className="flex flex-col md:flex-row justify-between gap-4">
                            ...
                            <div className="flex items-center ml-auto">
                              {appointment.status === 'confirmed' && (
                                <>
                                  {(() => {
                                    const { canJoin, message } = canJoinConsultation(appointment.appointment_date, appointment.urgent);
                                    return (
                              <Button 
                                variant="outline"
                                        className={`${canJoin 
                                          ...
                                          : 'bg-gray-100 text-gray-500'} ${appointment.urgent ? 'border-red-300' : 'border-medical-teal/30'}`}
